...
const PrintableMandato: React.FC<PrintableMandatoProps> = ({ client, fileConfig, asunto, mandatoBody, users, currentUser }) => {
    ...
    // Datos del Gestor/Agencia desde la configuración
    const agency = appSettings?.agency || {
        managerName: currentUser.name,
        managerDni: '__________________',
        managerColegiado: '__________________',
        name: 'Gestoría',
        address: '__________________'
    };

    // Fallbacks por si el nombre del responsable no está en la config de agencia pero sí en users
    const responsibleUserName = users.find(u => u.id === fileConfig.responsibleUserId)?.name || agency.managerName;
    ...
    const replacements: Record<string, string> = {
        ...
        '{{GESTOR_NAME}}': agency.managerName || responsibleUserName,
        '{{GESTOR_DNI}}': agency.managerDni || '__________________',
        '{{GESTOR_COLEGIADO_NUM}}': agency.managerColegiado || '__________________',
        '{{GESTOR_COLEGIO}}': 'GESTOR', // Podría añadirse a config si es necesario
        '{{GESTOR_DESPACHO}}': agency.name || '__________________',
        '{{GESTOR_DESPACHO_DIRECCION}}': agency.address || '__________________',
            ...
            <footer className="mt-4 space-y-4">
                <div>
                    <p className="text-center">
                        En {agency.address ? agency.address.split(',')[1] || 'ESPAÑA' : 'ESPAÑA'} a {day} de {month} de {year}
                    </p>
